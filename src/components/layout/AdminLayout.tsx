...
const AdminLayout = () => {
  const [sidebarOpenedAtLocationKey, setSidebarOpenedAtLocationKey] = useState<string | null>(null);
  const location = useLocation();
  const isSidebarOpen = sidebarOpenedAtLocationKey === location.key;

  const openSidebar = () => {
    setSidebarOpenedAtLocationKey(location.key);
  };

  const closeSidebar = () => {
    setSidebarOpenedAtLocationKey(null);
  };
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-neutral-200 bg-white px-3 min-[360px]:px-4 lg:px-8">
          ...
          <div className="flex items-center gap-2 min-[360px]:gap-4">
            <button
              type="button"
              className="p-2 text-neutral-500 hover:bg-neutral-100 rounded-lg relative"
              aria-label="알림"
            >
