...
import type { SellerOrderItemResponse, DeliveryInfoRequest, OrderItemStatus } from '@/types/order';
...
const SHIPPABLE_STATUSES: OrderItemStatus[] = ['PAYMENT_COMPLETED', 'PREPARING_SHIPMENT'];

// 판매자가 상태를 변경할 수 있는 상태 목록
const STATUS_TRANSITIONS: Partial<Record<OrderItemStatus, { label: string; next: OrderItemStatus }>> = {
...
export default function SellerOrderList() {
  ...
  const fetchItems = () => {
    setIsLoading(true);
    orderService.getSellerOrderItems()
      .then(setItems)
  ...
  const handleStatusChange = async (orderItemUuid: string, nextStatus: OrderItemStatus) => {
    try {
    ...
    <div className="bg-white rounded-2xl border border-neutral-200 p-6">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-4">
          {items.map((item) => {
            const statusInfo = ORDER_ITEM_STATUS_LABELS[item.itemStatus] ?? { text: item.itemStatus, className: 'bg-neutral-100 text-neutral-600' };
            const transition = STATUS_TRANSITIONS[item.itemStatus];
            const canInputDelivery = SHIPPABLE_STATUSES.includes(item.itemStatus);
            const isFormOpen = openDeliveryForm === item.orderItemUuid;

            return (
              <div key={item.orderItemUuid} className="border border-neutral-200 rounded-xl p-4 space-y-3">
                {/* 상품 정보 */}
                <div className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-sm font-medium text-neutral-900 line-clamp-2">{item.productName}</p>
                      <span className={`shrink-0 px-2 py-0.5 text-xs font-medium rounded-full ${statusInfo.className}`}>
                        {statusInfo.text}
                      </span>
                    </div>
                    <p className="text-sm font-bold text-neutral-900 mt-1">{formatPrice(item.productPrice)}</p>
                ...
                <div className="bg-neutral-50 rounded-lg p-3 text-xs text-neutral-600 space-y-1">
                  <p><span className="font-medium">수령인:</span> {item.recipient} ({item.contactNumber})</p>
                  <p><span className="font-medium">배송지:</span> {item.buyerAddress}</p>
                </div>

                {/* 등록된 운송장 */}
                {item.trackingNumber && (
                  <div className="bg-blue-50 rounded-lg p-3 text-xs text-blue-700 flex items-center justify-between">
                    <span><span className="font-medium">{item.carrierName}</span> {item.trackingNumber}</span>
                    <button
                      className="text-blue-500 hover:underline"
                      onClick={() => window.open(`https://search.naver.com/search.naver?query=${encodeURIComponent(item.carrierName ?? '')}+${encodeURIComponent(item.trackingNumber ?? '')}`, '_blank')}
                    >
                      조회
                    </button>
                  </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {transition && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleStatusChange(item.orderItemUuid, transition.next)}
                    >
                      {transition.label}
                    </Button>
                  )}
                ...
                {isFormOpen && (
                  <div className="border-t border-neutral-100 pt-3 space-y-2">
                    <p className="text-xs font-medium text-neutral-700">운송장 정보 입력</p>
                    <select
                      ...
                      onChange={(e) => {
                        const carrier = CARRIER_OPTIONS.find(c => c.code === e.target.value);
                        setDeliveryForm(f => ({
                          ...f,
